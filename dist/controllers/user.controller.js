...
const getSharedBudgetMembership = async (sharedBudgetId, userId) => {
...
const calculateSharedBudgetExpenseSummary = (expenses, monthlyBudget) => {
...
// ===============================
export const updateBudgetAllocations = async (req, res) => {
    try {
        const userId = req.user?.userId;
        if (!userId) {
            return res.status(401).json({
        ...
        const sanitizedAllocations = allocations.map((allocation) => ({
            category: typeof allocation.category === "string"
                ? allocation.category.trim().toLowerCase()
                : "",
            amount: Number(allocation.amount)
        }));
        const hasInvalidCategory = sanitizedAllocations.some((allocation) => !allowedBudgetCategories.includes(allocation.category));
        if (hasInvalidCategory) {
            return res.status(400).json({
                message: "One or more categories are invalid"
            });
        }
        const hasInvalidAmount = sanitizedAllocations.some((allocation) => Number.isNaN(allocation.amount) || allocation.amount < 0);
        if (hasInvalidAmount) {
            return res.status(400).json({
                message: "All allocation amounts must be valid non-negative numbers"
            });
        }
        const totalManagedBudget = sanitizedAllocations.reduce((sum, allocation) => sum + allocation.amount, 0);
        await prisma.$transaction([
            prisma.user.update({
                where: {
                    id: userId
...
export const updatePushSettings = async (req, res) => {
    try {
        ...
        const { pushToken, pushNotificationsEnabled, preferredLanguage } = req.body;
        const updateData = {};
...
export const createCustomCategory = async (req, res) => {
    try {
        ...
        const { name, icon, iconBackgroundColor } = req.body;
        ...
        const normalizedIconBackgroundColor = typeof iconBackgroundColor === "string" ? iconBackgroundColor.trim() : "";
        if (!normalizedName) {
            return res.status(400).json({
        ...
        const existingCategory = existingCategories.find((category) => category.name.trim().toLowerCase() === normalizedName.toLowerCase());
        if (existingCategory) {
...
// ===============================
export const inviteUserToSharedBudget = async (req, res) => {
    try {
        const userId = req.user?.userId;
        ...
        const hasPendingInvite = sharedBudget.invites.some((invite) => invite.invitedUserId === invitedUser.id);
        if (hasPendingInvite) {
            return res.status(400).json({
        ...
        await PushService.sendToUser(invitedUser.id, {
            title: invitedUser.preferredLanguage === "hebrew"
                ? "הזמנה לתקציב משותף"
                : "Shared Budget Invitation",
            body: invitedUser.preferredLanguage === "hebrew"
...
// ===============================
export const getMySharedBudgetInvites = async (req, res) => {
    try {
        const userId = req.user?.userId;
        if (!userId) {
            return res.status(401).json({
...
// ===============================
export const acceptSharedBudgetInvite = async (req, res) => {
    try {
        const userId = req.user?.userId;
        ...
        await PushService.sendToUser(invite.invitedByUser.id, {
            title: invite.invitedByUser.preferredLanguage === "hebrew"
                ? "ההזמנה אושרה"
                : "Invitation Accepted",
            body: invite.invitedByUser.preferredLanguage === "hebrew"
...
// ===============================
export const declineSharedBudgetInvite = async (req, res) => {
    try {
        const userId = req.user?.userId;
        const inviteId = req.params.inviteId;
        if (!userId) {
...
// ===============================
export const getMySharedBudgets = async (req, res) => {
    try {
        const userId = req.user?.userId;
        if (!userId) {
            return res.status(401).json({
...
// ===============================
export const getSharedBudgetDetail = async (req, res) => {
    try {
        const userId = req.user?.userId;
        const sharedBudgetId = req.params.sharedBudgetId;
        if (!userId) {
        ...
        const summary = calculateSharedBudgetExpenseSummary(sharedBudget.expenses, sharedBudgetMonthlyBudget);
        return res.status(200).json({
            sharedBudget,
            summary,
...
// ===============================
export const getSharedBudgetExpenses = async (req, res) => {
    try {
        const userId = req.user?.userId;
        const sharedBudgetId = req.params.sharedBudgetId;
        if (!userId) {
...
// ===============================
export const addSharedBudgetExpense = async (req, res) => {
    try {
        const userId = req.user?.userId;
        const sharedBudgetId = req.params.sharedBudgetId;
        if (!userId) {
        ...
        const { title, amount, category, mood, isNeed } = req.body;
        ...
        if (!allowedBudgetCategories.includes(normalizedCategory)) {
            return res.status(400).json({
