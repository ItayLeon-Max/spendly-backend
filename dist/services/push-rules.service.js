...
export class PushRulesService {
    static async runSmartPushCycle() {
        ...
        for (const user of users) {
            ...
            const shouldSkipForCooldown = user.lastPushSentAt &&
    ...
    static buildSmartMessageForUser(input) {
        const { preferredLanguage, monthlyBudget, expenses } = input;
        const now = new Date();
        const todayExpenses = expenses.filter((expense) => this.isSameDay(expense.date, now));
        const monthExpenses = expenses.filter((expense) => expense.date.getMonth() === now.getMonth() &&
            expense.date.getFullYear() === now.getFullYear());
        const weekExpenses = expenses.filter((expense) => this.isSameWeek(expense.date, now));
        ...
        const isEngagementWindow = currentHour >= ENGAGEMENT_PUSH_START_HOUR &&
    ...
    static randomMessage(language, type) {
        const messages = {
            english: {
                budget_exceeded: [
                    {
